...
class Patch2PointCrossAttn(nn.Module):
    ...
    def __init__(self, dim_in: int = 256, dim_out: int = 32, heads: int = 4,
                 sigma: float = 9.0, knn: int = 32, detach_dist: bool = True):
        super().__init__()
        assert dim_in % heads == 0, "dim_in must be divisible by heads"
        self.heads = heads
        ...
        self.sigma2 = float(sigma) ** 2 + 1e-12
        self.knn = int(max(1, knn))
        self.detach_dist = bool(detach_dist)

    def forward(self,
                point_xyz:  torch.Tensor,   # (B, N, 3) in [-1,1]
                patch_xyz:  torch.Tensor,   # (B, L, 3) in [-1,1]
        ...
        # content logits
        attn_logits = (q @ k.transpose(-2, -1)) * self.scale          # (B,H,N,L)

        d = torch.cdist(point_xyz, patch_xyz)                          # (B,N,L)
        if self.detach_dist:
            d = d.detach()
        pos_bias = -(d * d) / (2.0 * self.sigma2)                      # (B,N,L)
        attn_logits = attn_logits + pos_bias.unsqueeze(1)              # (B,H,N,L)

        k_eff = min(self.knn, L)
        with torch.no_grad():
            knn_idx = torch.topk(-d, k=k_eff, dim=-1).indices          # (B,N,k)
            keep_mask = torch.zeros((B, N, L), dtype=torch.bool, device=d.device)
            keep_mask.scatter_(dim=-1, index=knn_idx, value=True)      # True=keep
        attn_logits = attn_logits.masked_fill(~keep_mask.unsqueeze(1), float('-inf'))

        attn = attn_logits.softmax(dim=-1)                             # (B,H,N,L)
...
class BEVGenerator(nn.Module):
    def __init__(self, num_slices=6, grid_size=(160,160), x_range=(-60,60), y_range=(-60,60)):
        super().__init__()
...
class EnhancedBEVModule(nn.Module):
    ...
    def __init__(self, num_slices=6, grid_size=(160, 160),
        ...
        super().__init__()
        self.bev_generator   = BEVGenerator(num_slices=num_slices, grid_size=grid_size)  # [-1,1] 가정
    ...
    def forward(self, xyz: torch.Tensor):
        ...
        x = bev.view(B * S, 1, H, W)                                    # (B*S,1,H,W)
        patch_tok = self.patch_embed(x)                                  # (B*S, P, C)

        device = patch_tok.device
        py = (torch.arange(0, H, self.patch_size, device=device) + self.patch_size/2) / (H - 1)  # [0,1]
        px = (torch.arange(0, W, self.patch_size, device=device) + self.patch_size/2) / (W - 1)  # [0,1]
        gy, gx = torch.meshgrid(py, px, indexing='ij')
        grid_2d = torch.stack([gx, gy], dim=-1).view(1, P, 2)           # (1,P,2) in [0,1]
        grid_pe = grid_2d * 2.0 - 1.0                                    # (1,P,2) -> [-1,1]
        pos_emb = self.pos_mlp(grid_pe)                                   # (1,P,C)
        patch_tok = patch_tok + pos_emb                                   # (B*S,P,C), broadcast

        patch_tok = self.transformer_enc(patch_tok.transpose(0, 1)).transpose(0, 1)  # (B*S,P,C)
        patch_tok = patch_tok.view(B, S, P, self.embed_dim)              # (B,S,P,C)

        z_min = xyz[:, :, 2].amin(1, keepdim=True)                       # (B,1)
        z_max = xyz[:, :, 2].amax(1, keepdim=True)                       # (B,1)
        edges = z_min + (z_max - z_min) * torch.linspace(
            0, 1, self.num_slices + 1, device=xyz.device).view(1, -1)    # (B,S+1)
        z_mid = 0.5 * (edges[:, :-1] + edges[:, 1:])                     # (B,S)

        xs_base = grid_pe[..., 0:1].expand(B, -1, -1)                    # (B,P,1)
        ys_base = grid_pe[..., 1:2].expand(B, -1, -1)                    # (B,P,1)
        ...
        for s in range(S):
            ...
            zs = z_mid[:, s].view(B, 1, 1).expand(B, P, 1)               # (B,P,1)
            patch_xyz.append(torch.cat([xs_base, ys_base, zs], dim=-1))  # (B,P,3)

        return torch.cat(patch_feats, dim=1), torch.cat(patch_xyz, dim=1)  # (B,S*P,C), (B,S*P,3)
