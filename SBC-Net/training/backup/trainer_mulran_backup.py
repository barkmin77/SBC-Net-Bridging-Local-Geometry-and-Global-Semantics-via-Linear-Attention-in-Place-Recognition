...
from eval.pnv_evaluate_mulran import evaluate, pnv_write_eval_stats
...
def do_train(params: TrainingParams, saved_state_dict=None):
    ...
    for epoch in tqdm.tqdm(range(start_epoch + 1, params.epochs + 1)):
        ...
        # ---------- AFTER THE TRAINING STEP & SCHEDULER STEP ----------
        if epoch in (0, 1, 100, 200, 300, 350, 370, 400, 440, 445, 450, 500):
            model.eval()
            with torch.no_grad():
                prefix = f"{model_name}_epoch_{epoch:03d}"
                final_eval_stats = evaluate(model, device, params, log=False)
                mulran_recall_1p, cur_mean_recall_1 = \
                    pnv_write_eval_stats(f'./{model_name}.txt', prefix, final_eval_stats)
                print(f'------mulran_recall_1p: {mulran_recall_1p}, cur_mean_recall_1: {cur_mean_recall_1}------')

            if epoch >= 100:
                final_model_path = f"{model_pathname}_epoch_{epoch}.pth"
                checkpoint = {
