...
root_path = '/'.join(root_path.split('/')[:-2])  # 상위 상위 폴더까지 이동
...
from models.SBCnet import SBC_Net
import yaml
...
def evaluate(model, device, params: TrainingParams, log: bool = False, show_progress: bool = False):
    ...
    eval_database_files = [
        'crossrun_DCC_03_database.pickle',
        'crossrun_RiverSide_02_database.pickle',
        'crossrun_KAIST_01_database.pickle',
        'crossrun_KAIST_02_database.pickle',
        'crossrun_KAIST_03_database.pickle'
    ]

    eval_query_files = [
        'crossrun_DCC_03_query.pickle',
        'crossrun_RiverSide_02_query.pickle',
        'crossrun_KAIST_01_query.pickle',
        'crossrun_KAIST_02_query.pickle',
        'crossrun_KAIST_03_query.pickle'
    ]
    ...
    for database_file, query_file in zip(eval_database_files, eval_query_files):
        ...
        db_name = '_'.join(database_file.split('_')[1:3])      # 'DCC_03'
        query_name = '_'.join(query_file.split('_')[1:3])      # 'DCC_03'
        assert db_name == query_name, 'Database location: {} does not match query location: {}'.format(database_file, query_file)
        ...
        result = evaluate_dataset(model, device, params, database_sets, query_sets, log=log, show_progress=show_progress)
        stats[db_name] = result   # Save using 'DCC_03', 'RiverSide_02', etc.
...
def evaluate_dataset(model, device, params: TrainingParams, database_sets, query_sets, log: bool = False,
    ...
    pair_recall, pair_opr = get_recall(0, 0, database_embeddings, query_embeddings, query_sets, database_sets, log=log)
...
if __name__ == "__main__":
    ...
    if params.model_params.model == 'SBC_Net':
        model = SBC_Net(params.model_params)
    else:
    ...
    if args.weights is not None:
        ...
        model.load_state_dict(torch.load(args.weights, map_location=device, weights_only=False)['net'])
