...
class HardTripletMinerWithMasks:
    ...
    def mine(self, embeddings, positives_mask, negatives_mask):
        ...
        valid_pos_dist = dist_mat[a, p]
        valid_neg_dist = dist_mat[a, n]
        
        if valid_pos_dist.numel() > 0:
            self.mean_pos_pair_dist = valid_pos_dist.mean().item()
            self.max_pos_pair_dist = valid_pos_dist.max().item()
            self.min_pos_pair_dist = valid_pos_dist.min().item()
        else:
            self.mean_pos_pair_dist = 0

        if valid_neg_dist.numel() > 0:
            self.mean_neg_pair_dist = valid_neg_dist.mean().item()
            self.max_neg_pair_dist = valid_neg_dist.max().item()
            self.min_neg_pair_dist = valid_neg_dist.min().item()
        else:
            self.mean_neg_pair_dist = 0
